...
"""OpenAI adapter implementing the common LLM provider interface."""
...
class OpenAIProvider(LLMProvider):
    """Adapts the OpenAI client to the common LLM provider contract."""

    def __init__(self, client: Any, default_model: str | None = None) -> None:
        """
        Initialize the OpenAI provider.

        :param client: Configured OpenAI client used to execute requests.
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        """
        Execute an OpenAI request and normalize its response.
        ...
        if not model:
            return LLMResponse(
                ...
                model="",
                raw_text="",
                error="OpenAIProvider requires a model.",
            )
        ...
        try:
            ...
            logger.exception(
                "[OPENAI_PROVIDER] generate failed task=%s",
                request.task_name,
            )
            return LLMResponse(
                success=False,
    ...
    def _extract_text(response: Any) -> str:
        ...
        :param response: Raw response returned by the OpenAI client.
