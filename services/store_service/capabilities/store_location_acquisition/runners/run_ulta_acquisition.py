# services/store_service/capabilities/store_location_acquisition/runners/run_ulta_acquisition.py

"""Executable runner for Ulta Beauty store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.ulta_acquisition_strategy import (
    UltaAcquisitionStrategy,
)


ULTA_OUTPUT_ROOT = OUTPUT_ROOT / "ulta"
...
def _build_csv_row(
    ...
    """
    Convert an Ulta payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Ulta records to a CSV file.
...
def main() -> None:
    """Run the Ulta acquisition and write its output artifacts."""
    strategy = UltaAcquisitionStrategy(
        state_workers=12,
        city_workers=24,
        store_workers=32,
        request_timeout=20,
    ...
    print()
    print("=" * 72)
    print("Ulta Beauty Acquisition")
    print("=" * 72)
    print("Source: https://www.ulta.com/stores/directory")
    print(
        "Method: Playwright directory rendering + "
        "requests / BeautifulSoup store pages"
    )
    print(
        "Hierarchy: directory -> state sections -> "
        "store detail pages"
    )
    print(
        "Dedup key: retailer_store_id from "
        "Book Appointment URL or store page slug"
    )
    print(
        "Workers: "
        f"state={strategy.state_workers}, "
        f"store-fetch={strategy.city_workers}, "
        f"store-parse={strategy.store_workers}"
    )
    print()
    ...
    csv_path = (
        output_dir
        / f"{run_id}_ulta_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_ulta_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Ulta Beauty Acquisition Completed")
    print("=" * 72)
