# services/store_service/capabilities/store_location_acquisition/runners/run_petco_acquisition.py

"""Executable runner for Petco store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.petco_acquisition_strategy import (
    PetcoAcquisitionStrategy,
)


PETCO_OUTPUT_ROOT = OUTPUT_ROOT / "petco"
...
def _build_csv_row(
    ...
    """
    Convert a Petco payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Petco records to a CSV file.
...
def main() -> None:
    """Run the Petco acquisition and write its output artifacts."""
    strategy = PetcoAcquisitionStrategy(
        state_workers=12,
        city_workers=24,
        store_workers=32,
        request_timeout=20,
    ...
    print()
    print("=" * 72)
    print("Petco Acquisition")
    print("=" * 72)
    print("Source: https://stores.petco.com/us")
    print("Method: HTML / BeautifulSoup")
    print(
        "Hierarchy: state pages -> city pages -> "
        "store cards/detail pages"
    )
    print(
        "Dedup key: retailer_store_id from "
        "View Details URL suffix"
    )
    print(
        "Workers: "
    ...
    output_dir = PETCO_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_petco_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_petco_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Petco Acquisition Completed")
    print("=" * 72)
    print(
