# services/store_service/capabilities/store_location_acquisition/runners/run_target_acquisition.py

"""Executable runner for Target store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.target_acquisition_strategy import (
    TargetAcquisitionStrategy,
)
...
def _build_csv_row(
    ...
    """
    Convert a Target payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Target records to a CSV file.
...
def main() -> None:
    """Run the Target acquisition and write its output artifacts."""
    strategy = TargetAcquisitionStrategy(
        state_workers=24,
    ...
    print("=" * 72)
    print(f"Source: {source_info.store_locator_url}")
    print(
        "Method: requests / BeautifulSoup + Playwright "
        "for multi-store city discovery"
    )
    print(
        "Hierarchy: directory -> state pages -> single-store city / "
        "multi-store city -> store detail pages"
    )
    print(
        "Dedup key: retailer_store_id from canonical "
        "/sl/<slug>/<store_id> URL"
    )
    print(
        "Workers: "
        f"state={strategy.state_workers}, "
        f"playwright={strategy.playwright_workers}, "
    ...
    output_dir = TARGET_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_target_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_target_summary.json"
    )
