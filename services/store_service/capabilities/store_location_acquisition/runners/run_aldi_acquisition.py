# services/store_service/capabilities/store_location_acquisition/runners/run_aldi_acquisition.py

"""Executable runner for ALDI store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.aldi_acquisition_strategy import (
    AldiAcquisitionStrategy,
)


ALDI_OUTPUT_ROOT = OUTPUT_ROOT / "aldi"
...
def _build_csv_row(
    ...
    """
    Convert an ALDI payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized ALDI records to a CSV file.
...
def main() -> None:
    """Run the ALDI acquisition and write its output artifacts."""
    strategy = AldiAcquisitionStrategy(
        request_timeout=60,
        max_retries=3,
    )
    ...
    print()
    print("=" * 72)
    print("ALDI Acquisition")
    print("=" * 72)
    print("Source: https://info.aldi.us/stores")
    print(
        "Provider: Uberall "
        "(official ALDI Store Finder location dataset)"
    )
    print("Method: single /locations/all JSON request")
    print("Dedup key: API identifier field")
    ...
    output_dir = ALDI_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_aldi_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_aldi_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("ALDI Acquisition Completed")
    print("=" * 72)
    print(
