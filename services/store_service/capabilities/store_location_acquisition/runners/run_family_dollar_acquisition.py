# services/store_service/capabilities/store_location_acquisition/runners/run_family_dollar_acquisition.py

"""Executable runner for Family Dollar store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.family_dollar_acquisition_strategy import (
    FamilyDollarAcquisitionStrategy,
)


FAMILY_DOLLAR_OUTPUT_ROOT = OUTPUT_ROOT / "family_dollar"
...
def _build_csv_row(
    ...
    """
    Convert a Family Dollar payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Family Dollar records to a CSV file.
...
def main() -> None:
    """Run the Family Dollar acquisition and write its output artifacts."""
    strategy = FamilyDollarAcquisitionStrategy(
        state_workers=8,
        city_workers=16,
        store_workers=32,
        request_timeout=20,
    ...
    print()
    print("=" * 72)
    print("Family Dollar Acquisition")
    print("=" * 72)
    print("Source: https://locations.familydollar.com/index.html")
    print("Method: HTML / BeautifulSoup")
    print(
        "Hierarchy: state index -> state city list -> "
        "city store cards"
    )
    print("Dedup key: retailer_store_id")
    print()
    ...
    output_dir = FAMILY_DOLLAR_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_family_dollar_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_family_dollar_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Family Dollar Acquisition Completed")
    print("=" * 72)
    print(
