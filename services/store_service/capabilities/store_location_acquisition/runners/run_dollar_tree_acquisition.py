# services/store_service/capabilities/store_location_acquisition/runners/run_dollar_tree_acquisition.py

"""Executable runner for Dollar Tree store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.dollar_tree_acquisition_strategy import (
    DollarTreeAcquisitionStrategy,
)


DOLLAR_TREE_OUTPUT_ROOT = OUTPUT_ROOT / "dollar_tree"
...
def _build_csv_row(
    ...
    """
    Convert a Dollar Tree payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Dollar Tree records to a CSV file.
...
def main() -> None:
    """Run the Dollar Tree acquisition and write its output artifacts."""
    strategy = DollarTreeAcquisitionStrategy(
        state_workers=8,
        city_workers=16,
        store_workers=32,
        request_timeout=20,
    ...
    print()
    print("=" * 72)
    print("Dollar Tree Acquisition")
    print("=" * 72)
    print("Source: https://locations.dollartree.com/index.html")
    ...
    output_dir = DOLLAR_TREE_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_dollar_tree_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_dollar_tree_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Dollar Tree Acquisition Completed")
    print("=" * 72)
    print(
    ...
    if validation.duplicate_store_ids:
        print(
            "Duplicate store IDs: "
            f"{', '.join(validation.duplicate_store_ids)}"
        )
