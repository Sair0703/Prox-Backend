# services/store_service/capabilities/store_location_acquisition/runners/run_albertsons_acquisition.py

"""Executable runner for Albertsons store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.albertsons_acquisition_strategy import (
    AlbertsonsAcquisitionStrategyV2,
)


ALBERTSONS_OUTPUT_ROOT = OUTPUT_ROOT / "albertsons"
...
def _build_csv_row(
    ...
    """
    Convert an Albertsons payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Albertsons records to a CSV file.
...
def main() -> None:
    """Run the Albertsons acquisition and write its output artifacts."""
    strategy = AlbertsonsAcquisitionStrategyV2(
        state_workers=16,
        location_workers=16,
        retry_workers=8,
        parse_workers=32,
    ...
    print()
    print("=" * 72)
    print("Albertsons Acquisition v2")
    print("=" * 72)
    print("Strategy: AlbertsonsAcquisitionStrategyV2")
    print(
        "Strategy path: "
        "strategies/albertsons_acquisition_strategy.py"
    )
    print(
        "Source: https://local.albertsons.com/index.html"
    )
    ...
    print(
        "Store ID: Weekly Ad URL storeId parameter"
    )
    print(
        "Coordinates: Schema.org geo meta on store detail pages"
    )
    print(
        "Workers: "
        f"state={strategy.state_workers}, "
        f"location={strategy.location_workers}, "
        f"retry={strategy.retry_workers}, "
    ...
    print(
        ...
        f"deferred delay={strategy.retry_round_delay}s"
    )
    print()
    ...
    output_dir = ALBERTSONS_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_albertsons_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_albertsons_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Albertsons Acquisition Completed")
    print("=" * 72)
    print(
