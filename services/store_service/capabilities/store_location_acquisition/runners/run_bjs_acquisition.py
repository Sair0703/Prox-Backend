# services/store_service/capabilities/store_location_acquisition/runners/run_bjs_acquisition.py

"""Executable runner for BJ's Wholesale Club store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.bjs_acquisition_strategy import (
    BjsAcquisitionStrategy,
)


BJS_OUTPUT_ROOT = OUTPUT_ROOT / "bjs"
...
def _build_csv_row(
    ...
    """
    Convert a BJ's payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized BJ's Wholesale Club records to a CSV file.
...
def main() -> None:
    """Run the BJ's Wholesale Club acquisition and write its output artifacts."""
    strategy = BjsAcquisitionStrategy(
        headless=False,
        state_workers=1,
        town_workers=1,
        store_workers=4,
    ...
    print()
    print("=" * 72)
    print("BJ's Wholesale Club Acquisition")
    print("=" * 72)
    print(
        f"Source: {source_info.store_locator_url}"
    )
    print(
        "Method: Playwright locator discovery + "
        "Playwright detail capture + BeautifulSoup parsing"
    )
    print(
        "Hierarchy: root -> state dropdown -> "
        "town dropdown -> detail page"
    )
    print(
        "Dedup key: retailer_store_id from trailing "
        "four-digit suffix in detail URL"
    )
    print(
        "Workers: "
        f"state={strategy.state_workers}, "
        f"town={strategy.town_workers}, "
    ...
    output_dir = BJS_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_bjs_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_bjs_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("BJ's Wholesale Club Acquisition Completed")
    print("=" * 72)
    print(
