# services/store_service/capabilities/store_location_acquisition/runners/run_petsmart_acquisition.py

"""Executable runner for PetSmart store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.petsmart_acquisition_strategy import (
    PetSmartAcquisitionStrategy,
)


PETSMART_OUTPUT_ROOT = OUTPUT_ROOT / "petsmart"
...
def _build_csv_row(
    ...
    """
    Convert a PetSmart payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized PetSmart records to a CSV file.
...
def main() -> None:
    """Run the PetSmart acquisition and write its output artifacts."""
    strategy = PetSmartAcquisitionStrategy(
        state_workers=12,
        city_workers=24,
        store_workers=32,
        request_timeout=20,
    ...
    print(
        "Hierarchy: directory -> state pages -> "
        "city pages -> store cards"
    )
    print(
        "Dedup key: retailer_store_id from "
        "store details URL regex fallback"
    )
    print()
    ...
    output_dir = PETSMART_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_petsmart_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_petsmart_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("PetSmart Acquisition Completed")
    print("=" * 72)
    print(
