# services/store_service/capabilities/store_location_acquisition/runners/run_costco_acquisition.py

"""Executable runner for Costco store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.costco_acquisition_strategy import (
    CostcoAcquisitionStrategyV2,
)


COSTCO_OUTPUT_ROOT = OUTPUT_ROOT / "costco"
...
def _build_csv_row(
    ...
    """
    Convert a Costco payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Costco records to a CSV file.
...
def main() -> None:
    """Run the Costco acquisition and write its output artifacts."""
    strategy = CostcoAcquisitionStrategyV2(
        state_workers=16,
        store_workers=16,
        retry_workers=8,
        parse_workers=32,
        request_timeout=30,
        per_request_attempts=1,
        retry_rounds=3,
        retry_round_delays=(15.0, 45.0, 90.0),
        retry_round_workers=(8, 4, 2),
    ...
    print()
    print("=" * 72)
    print("Costco Acquisition v2")
    print("=" * 72)
    print("Strategy:", strategy.__class__.__name__)
    print(
        "Source: "
        "https://www.costco.com/sitemaps/warehouses-by-state"
    )
    print("Method: requests + BeautifulSoup")
    print(
        "Hierarchy: state sitemap -> "
        "direct warehouse detail pages"
    )
    print(
        "Store ID: final path segment of "
        "canonical warehouse URL"
    )
    ...
    print(
        "Request attempts per round: "
        f"{strategy.per_request_attempts}"
    )
    print(
        "Deferred rounds: "
        f"{strategy.retry_rounds}"
    )
    print(
        "Deferred delays: "
        f"{strategy.retry_round_delays}"
    )
    print(
        "Deferred workers: "
        f"{strategy.retry_round_workers}"
    )
    print()
    ...
    output_dir = COSTCO_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_costco_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_costco_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Costco Acquisition Completed")
    print("=" * 72)
    print(
