# services/store_service/capabilities/store_location_acquisition/runners/run_grocery_outlet_acquisition.py

"""Executable runner for Grocery Outlet store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.grocery_outlet_acquisition_strategy import (
    GroceryOutletCsvAcquisitionStrategy,
)


GROCERY_OUTLET_OUTPUT_ROOT = OUTPUT_ROOT / "grocery_outlet"
...
def _build_csv_row(
    ...
    """
    Convert a Grocery Outlet payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Grocery Outlet records to a CSV file.
...
def main() -> None:
    """Run the Grocery Outlet acquisition and write its output artifacts."""
    csv_path = (
        PROJECT_ROOT
        / "services/store_service/capabilities/"
        "store_location_acquisition/input/Grocery_Outlet_2023.csv"
    )

    strategy = GroceryOutletCsvAcquisitionStrategy(
        csv_path=csv_path
    )
    ...
    print()
    print("=" * 72)
    print("Grocery Outlet Acquisition")
    print("=" * 72)
    print(f"Source: {csv_path}")
    print(
        "Method: local CSV parsed from Google My Maps export"
    )
    print(
        "Hierarchy: CSV rows -> normalized store payloads"
    )
    print(
        "Dedup key: synthetic retailer_store_id "
        "derived from location fields"
    )
    print()
    ...
    output_dir = GROCERY_OUTLET_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    output_csv_path = (
        output_dir
        / f"{run_id}_grocery_outlet_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_grocery_outlet_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Grocery Outlet Acquisition Completed")
    print("=" * 72)
    print(
