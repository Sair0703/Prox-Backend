# services/store_service/capabilities/store_location_acquisition/runners/run_gelsons_acquisition.py

"""Executable runner for Gelson's store location acquisition."""
...
STRATEGY_PATH = (
    ...
    / "strategies"
    / "gelsons_acquisition_strategy.py"
)
...
GELSONS_OUTPUT_ROOT = OUTPUT_ROOT / "gelsons"
...
# Load the fixed Gelson's strategy directly from its expected source file.
if not STRATEGY_PATH.exists():
    raise FileNotFoundError(
        f"Fixed Gelson's strategy not found: {STRATEGY_PATH}"
    )

spec = spec_from_file_location(
    "gelsons_acquisition_strategy_v2_fixed_runtime",
    STRATEGY_PATH,
)

if spec is None or spec.loader is None:
    raise RuntimeError(
        f"Unable to load fixed Gelson's strategy: {STRATEGY_PATH}"
    )

module = module_from_spec(spec)
spec.loader.exec_module(module)
GelsonsAcquisitionStrategyV2 = module.GelsonsAcquisitionStrategyV2
...
def _build_csv_row(
    ...
    """
    Convert a Gelson's payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Gelson's records to a CSV file.
...
def main() -> None:
    """Run the Gelson's acquisition and write its output artifacts."""
    strategy = GelsonsAcquisitionStrategyV2(
        request_timeout=30,
        expected_store_count=28,
    )
    ...
    print()
    print("=" * 72)
    print("Gelson's Acquisition v2 FIXED")
    print("=" * 72)
    print("Strategy:", strategy.__class__.__name__)
    print("Strategy path:", STRATEGY_PATH)
    print("Source: https://www.gelsons.com/stores")
    print("Method: requests + BeautifulSoup")
    print(
        "Hierarchy: single store locator HTML page -> "
        "store entries"
    )
    print(
        "Store ID: not exposed in observed HTML; left empty"
    )
    print("Store detail pages: not required")
    print()
    ...
    output_dir = GELSONS_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_gelsons_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_gelsons_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Gelson's Acquisition Completed")
    print("=" * 72)
    print(
