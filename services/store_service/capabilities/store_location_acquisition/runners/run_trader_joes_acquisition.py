# services/store_service/capabilities/store_location_acquisition/runners/run_trader_joes_acquisition.py

"""Executable runner for Trader Joe's store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.trader_joes_acquisition_strategy import (
    TraderJoesAcquisitionStrategy,
)


TRADER_JOES_OUTPUT_ROOT = OUTPUT_ROOT / "trader_joes"
...
def _build_csv_row(
    ...
    """
    Convert a Trader Joe's payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Trader Joe's records to a CSV file.
...
def main() -> None:
    """Run the Trader Joe's acquisition and write its output artifacts."""
    strategy = TraderJoesAcquisitionStrategy(
        state_workers=16,
        city_workers=48,
        parse_workers=64,
        request_timeout=30,
    ...
    print()
    print("=" * 72)
    print("Trader Joe's Acquisition")
    print("=" * 72)
    print("Source: https://locations.traderjoes.com/")
    print("Method: requests + BeautifulSoup")
    print(
        "Hierarchy: root -> state pages -> "
        "city pages -> store listings"
    )
    print(
        "Dedup key: numeric final path segment "
        "of canonical store URL"
    )
    print(
        "Workers: "
    ...
    csv_path = (
        output_dir
        / f"{run_id}_trader_joes_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_trader_joes_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Trader Joe's Acquisition Completed")
    print("=" * 72)
    print(
