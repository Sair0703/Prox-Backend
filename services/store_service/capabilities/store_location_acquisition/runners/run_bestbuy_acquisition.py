# services/store_service/capabilities/store_location_acquisition/runners/run_bestbuy_acquisition.py

"""Executable runner for Best Buy store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.bestbuy_acquisition_strategy import (
    BestBuyAcquisitionStrategy,
)


BESTBUY_OUTPUT_ROOT = OUTPUT_ROOT / "bestbuy"
...
def _build_csv_row(
    ...
    """
    Convert a Best Buy payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Best Buy records to a CSV file.
...
def main() -> None:
    """Run the Best Buy acquisition and write its output artifacts."""
    strategy = BestBuyAcquisitionStrategy(
        state_workers=12,
        city_workers=24,
        store_workers=32,
        request_timeout=20,
    ...
    print()
    print("=" * 72)
    print("Best Buy Acquisition")
    print("=" * 72)
    print("Source: https://stores.bestbuy.com/")
    print("Method: HTML / BeautifulSoup")
    print(
        "Hierarchy: root -> state pages -> "
        "city pages / store detail pages"
    )
    print(
        "Dedup key: retailer_store_id from trailing "
        "numeric suffix in detail URL"
    )
    print(
        "Workers: "
    ...
    output_dir = BESTBUY_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_bestbuy_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_bestbuy_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Best Buy Acquisition Completed")
    print("=" * 72)
    print(
