# services/store_service/capabilities/store_location_acquisition/runners/run_kroger_acquisition.py

"""Executable runner for Kroger store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.kroger_acquisition_strategy import (
    KrogerAcquisitionStrategyV5,
)


KROGER_OUTPUT_ROOT = OUTPUT_ROOT / "kroger"
...
def _build_csv_row(
    ...
    """
    Convert a Kroger payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized Kroger records to a CSV file.
...
def main() -> None:
    """Run the Kroger acquisition and write its output artifacts."""
    strategy = KrogerAcquisitionStrategyV5(
        city_workers=48,
        locator_workers=16,
        parse_workers=32,
        request_timeout=30,
    ...
    print()
    print("=" * 72)
    print("Kroger Acquisition v5")
    print("=" * 72)
    print(
        "Method: official JSON APIs + requests + "
        "browser API fallback on 403"
    )
    print(
        "Hierarchy: grocery-stores.json -> city JSON -> "
        "locationIds -> full Store Info"
    )
    print("Playwright: API fallback only")
    print("HTML scraping: not required")
    print(
        "Workers: "
        f"city={strategy.city_workers}, "
        f"locator={strategy.locator_workers}, "
    ...
    output_dir = KROGER_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_kroger_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_kroger_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("Kroger Acquisition Completed")
    print("=" * 72)
    print(
