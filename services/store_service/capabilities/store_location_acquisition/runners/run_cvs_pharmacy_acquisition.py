# services/store_service/capabilities/store_location_acquisition/runners/run_cvs_pharmacy_acquisition.py

"""Executable runner for CVS Pharmacy store location acquisition."""
...
from services.store_service.capabilities.store_location_acquisition.strategies.cvs_pharmacy_acquisition_strategy import (
    CVSAcquisitionStrategy,
)


CVS_PHARMACY_OUTPUT_ROOT = OUTPUT_ROOT / "cvs_pharmacy"
...
def _build_csv_row(
    ...
    """
    Convert a CVS Pharmacy payload into the shared acquisition CSV schema.
...
def _write_csv(
    ...
    """
    Write normalized CVS Pharmacy records to a CSV file.
...
def main() -> None:
    """Run the CVS Pharmacy acquisition and write its output artifacts."""
    strategy = CVSAcquisitionStrategy(
        state_workers=16,
        city_workers=32,
    )
    ...
    print()
    print("=" * 72)
    print("CVS Pharmacy Acquisition")
    print("=" * 72)
    print(
    ...
    output_dir = CVS_PHARMACY_OUTPUT_ROOT / run_id
    output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    csv_path = (
        output_dir
        / f"{run_id}_cvs_pharmacy_us_locations.csv"
    )
    summary_path = (
        output_dir
        / f"{run_id}_cvs_pharmacy_summary.json"
    )
    ...
    print()
    print("=" * 72)
    print("CVS Pharmacy Acquisition Completed")
    print("=" * 72)
    print(
