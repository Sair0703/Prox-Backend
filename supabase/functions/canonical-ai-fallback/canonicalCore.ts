/**
 * Canonical name matching core - shared normalization helpers.
 * Keep this file in sync with canonical-name-prod/canonicalCore.ts.
 */
...
export interface CanonicalLookup {
  ...
  byBrand: Map<string, CanonicalEntry[]>;
}
...
const wordDigitRe =
  /\b([a-z]{2,})\s+(\d)\b(?!\s*(?:oz|fl(?:\s*oz)?|lb|lbs|g|kg|ml|l|ct|count|pack|pk|qt|pt|gal|each|pkg)\b)/gi;
...
export async function loadCanonicalLookup(supabase: SupabaseClient, tableName: string): Promise<CanonicalLookup> {
  ...
  const byBrand = new Map<string, CanonicalEntry[]>();
  let offset = 0;
  const PAGE = 1000;
  ...
  while (true) {
    ...
    if (error) {
      throw new Error(`Could not load canonical data from ${tableName}: ${error.message}`);
    }
    ...
    for (const row of data) {
      ...
      const brandEntries = byBrand.get(row.brand) || [];
      brandEntries.push(entry);
      byBrand.set(row.brand, brandEntries);
    }
  ...
  return { entries, byBrandAndSignature, byBrand };
}
...
export function lookupBrandCandidates(lookup: CanonicalLookup, brand: string): CanonicalEntry[] {
  return lookup.byBrand.get(brand) || [];
}
